export class LastFmService {
    constructor(apiKey, apiSecret, supabase) {
        this.apiKey = apiKey;
    ...
    async getRecentTracks(username, limit = 50) {
        try {
    ...
    async storeCurrentTrack(currentTrack) {
        try {
    ...
    async storeScrobbles(username, limit = 100) {
        try {
            ...
            const scrobbles = (Array.isArray(tracks) ? tracks : [tracks])
                .filter((track) => !track['@attr']?.nowplaying) // exclude now playing
                .map((track) => ({
                track_title: track.name || '',
    ...
    async getStoredScrobbles(limit = 50, offset = 0) {
        try {
            const { data, error } = await this.supabase
                .from('music_scrobbles')
                .select('*')
