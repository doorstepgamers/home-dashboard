...
app.post('/api/victron-devices', async (req, res) => {
    if (!bluetoothManager) {
        return res.status(500).json({ error: 'Bluetooth manager not initialized' });
    }
    try {
        ...
        const device = await bluetoothManager.registerDevice(device_name, mac_address, device_type, pin_code);
...
app.delete('/api/victron-devices/:id', async (req, res) => {
    if (!bluetoothManager) {
        return res.status(500).json({ error: 'Bluetooth manager not initialized' });
    }
    try {
...
app.get('/api/victron-devices/discovery/logs', async (req, res) => {
    if (!bluetoothManager) {
        return res.status(500).json({ error: 'Bluetooth manager not initialized' });
    }
    try {
